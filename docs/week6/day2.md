...
LangChain 不是唯一选择。LlamaIndex 专注于 RAG 场景，如果你的核心需求是文档检索和问答，LlamaIndex 可能更合适：

| 框架 | 适用场景 | 特点 |
|------|---------|------|
| LangChain | 通用 Agent 开发 | 生态丰富、灵活 |
| LlamaIndex | RAG 文档问答 | 检索优化、简单易用 |
| Haystack | 企业级 NLP 流水线 | 生产级、可扩展 |

> 初学者建议先学 LangChain（本课程选择），后续根据项目需求再学习其他框架。

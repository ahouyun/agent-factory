...
Agent Skills 和 MCP 都是扩展 Agent 能力的方式，但设计理念不同：

| 维度 | Agent Skills | MCP |
|------|-------------|-----|
| 本质 | Agent 的"技能包" | Agent 的"工具接口" |
| 粒度 | 高层能力（如"写代码"） | 底层操作（如"读文件"） |
| 实现 | Prompt + 工具组合 | 标准化协议 |
| 来源 | 开发者自定义 | 社区/官方提供 |
| 适用 | 特定任务场景 | 通用工具集成 |
...
> 两者可以结合使用：用 MCP 提供底层工具，用 Skills 编排高层任务。
